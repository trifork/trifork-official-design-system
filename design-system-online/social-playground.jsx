...
const BLANK = {
  event:  { variant: 'event',  layout: 'bottom', bg: 'photo', photoSrc: 'assets/imagery/trifork-stock-1.jpg', photoPos: '60% 35%', eyebrow: 'JOIN US', eyebrowColor: 'auto', title: 'AI Enablement: Boost your business with artificial intelligence', body: '', meta: ['20 February, Aarhus', '15:30–17:30'], metaRows: [], positions: [], speakers: [{ name: 'Sune Lundorff', role: 'Head of Department', company: 'OK' }], showLogo: false },
  ...
  news:   { variant: 'news',   layout: 'bottom', bg: 'photo', photoSrc: 'assets/imagery/trifork-aarhus-1.jpg', photoPos: 'center', eyebrow: '5 MAY 2026', eyebrowColor: 'auto', title: 'Q1 Report 2026', body: '', meta: [], metaRows: [{ label: 'Report', value: '07:00 (CEST)' }, { label: 'Webcast', value: '11:00–12:00 (CEST)' }], positions: [], showLogo: false },
  case:   { variant: 'case',   layout: 'bottom', bg: 'photo', photoSrc: 'assets/imagery/trifork-vision-pro.png', photoPos: 'center', eyebrow: 'CASE · AVIATION', eyebrowColor: 'auto', title: 'Cutting recurrent-training cost by 38% for a European carrier.', body: '', meta: ['Read the case'], metaRows: [], positions: [], showLogo: false },
};

function blankPage(type) { return { id: uid(), ...JSON.parse(JSON.stringify(BLANK[type] || BLANK.event)) }; }
...
function pageToProps(page, format) {
  return {
    ...
    quoteAttribution: page.variant === 'quote' ? { name: page.quoteName, role: page.quoteRole, avatar: page.quoteAvatar || undefined } : undefined,
...
async function renderAndCapture(props, dims) {
  ...
  await new Promise((res) => { root.render(<SocialPost {...props} scale={1} />); requestAnimationFrame(() => requestAnimationFrame(res)); });
  await Promise.all([...holder.querySelectorAll('img')].map((img) => (img.complete && img.naturalWidth) ? Promise.resolve() : new Promise((r) => { img.onload = img.onerror = r; })));
  if (document.fonts && document.fonts.ready) { try { await document.fonts.ready; } catch (e) {} }
  await new Promise((r) => setTimeout(r, 160));
  const node = holder.firstChild;
  const blob = await htmlToImage.toBlob(node, { width: dims.w, height: dims.h, pixelRatio: 1, cacheBust: true, backgroundColor: props.bg === 'light' ? '#D5E5ED' : '#2C3A42' });
  root.unmount();
  holder.remove();
  return blob;
}
...
function Playground() {
  ...
  const addPage = () => { const np = { ...JSON.parse(JSON.stringify(page)), id: uid() }; setPages((ps) => { const n = [...ps]; n.splice(sel + 1, 0, np); return n; }); setSel(sel + 1); };
  const delPage = () => { if (pages.length <= 1) return; setPages((ps) => ps.filter((_, i) => i !== sel)); setSel(Math.max(0, sel - 1)); };
  const move = (dir) => { const j = sel + dir; if (j < 0 || j >= pages.length) return; setPages((ps) => { const n = [...ps]; const [it] = n.splice(sel, 1); n.splice(j, 0, it); return n; }); setSel(j); };
  const changeType = (e) => { const t = e.target.value; setPages((ps) => ps.map((p, i) => i === sel ? { id: p.id, ...JSON.parse(JSON.stringify(BLANK[t])) } : p)); };
  ...
  React.useEffect(() => {
    ...
    const PAD = 64;
    const recalc = () => { if (!stageRef.current) return; const w = stageRef.current.clientWidth - PAD; const h = stageRef.current.clientHeight - PAD; const s = Math.min(w / dims.w, h / dims.h); setScale(s > 0 ? s : 0.4); };
    recalc();
  ...
  const downloadOne = async () => {
    try { setBusy('Rendering…'); const blob = await renderAndCapture(pageToProps(page, format), dims); downloadBlob(blob, `trifork-${page.variant}-${sel + 1}.png`); setBusy(''); }
    catch (err) { setBusy(''); alert('Could not generate image: ' + err.message); }
  };
  const downloadAll = async () => {
    try {
      setBusy('Rendering 1/' + pages.length + '…');
      if (pages.length === 1) { const blob = await renderAndCapture(pageToProps(pages[0], format), dims); downloadBlob(blob, `trifork-${pages[0].variant}.png`); setBusy(''); return; }
      if (typeof JSZip === 'undefined') throw new Error('JSZip not loaded');
      const zip = new JSZip();
      for (let i = 0; i < pages.length; i++) { setBusy('Rendering ' + (i + 1) + '/' + pages.length + '…'); const blob = await renderAndCapture(pageToProps(pages[i], format), dims); zip.file(`page-${String(i + 1).padStart(2, '0')}.png`, blob); }
      setBusy('Zipping…');
